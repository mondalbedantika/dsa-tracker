...
type Store = { problems: Problem[]; problemSets: ProblemSet[]; activities: Activity[]; profile: Profile; theme: Theme; toggleTheme:()=>void; markSolved: (id:string)=>void; updateNotes:(id:string, notes:string)=>void; advanceRevision:(id:string)=>void; updateProfile:(profile:Profile)=>void; updateGoal:(goal:string)=>void };
const seed: Problem[] = [
...
export const AppProvider=({children}:{children:ReactNode})=>{const [data]=useState(()=>{try{return JSON.parse(localStorage.getItem(key)||'') }catch{return null}}); const [problems,setProblems]=useState<Problem[]>(data?.problems||seed); const [profile,setProfile]=useState<Profile>(data?.profile||defaultProfile); const [theme,setTheme]=useState<Theme>(data?.theme||'light');
useEffect(()=>{localStorage.setItem(key,JSON.stringify({problems,profile,theme}));document.documentElement.dataset.theme=theme},[problems,profile,theme]);
...
return <C.Provider value={{problems,problemSets:sets,activities:[],profile,theme,toggleTheme:()=>setTheme(t=>t==='light'?'dark':'light'),markSolved,updateNotes,advanceRevision,updateProfile:setProfile,updateGoal:(goal)=>setProfile(p=>({...p,goal}))}}>{children}</C.Provider>};
